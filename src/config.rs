...
fn default_journal_path() -> PathBuf {
    expand_tilde("~/Documents/VoiceJournal.md")
}
...
journal_path = "/tmp/j.md""#,
    ...
    fn env_override_changes_config_path() {
        ...
        // SAFETY: no other test in this crate reads VOICE_JOURNAL_CONFIG.
        unsafe { std::env::set_var("VOICE_JOURNAL_CONFIG", &path) };
        assert_eq!(config_path().unwrap(), path);
        unsafe { std::env::remove_var("VOICE_JOURNAL_CONFIG") };
    }
}
